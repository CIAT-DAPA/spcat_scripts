...
def import_accessions(data = None):
    ...
    if not len(accession_files) == 0:
        ...
        for file in accession_file_names:
            ...
            try:
                accessions = read_file( data, file, c.cols_accession)
            ...
            accessions.fillna('', inplace=True)
        ...
        print('No file ending in accession.csv has been found.')


def import_attributes(data = None):
    ...
    if not len(attributes_files) == 0:
        ...
        for file in attributes_file_names:
            ...
            try:
                attributes = read_file( data, file, c.cols_attribute)
            except ValueError as e:
                print(f"Error reading {file} file: {str(e)}")
                continue
            ...
            # Traverse each row of the DataFrame
            for index, row in tqdm(attributes.iterrows(), total=len(attributes)):
